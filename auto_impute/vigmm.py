...
class VIGMM(Model):
    ...
    def __init__(self, data, num_gaussians, verbose=None, α0=None, m0=None, β0=None, W0=None, ν0=None):
        ...
        self.γs = [np.array([1, self.α0])]*self.num_gaussians
    ...
    def fit(self, max_iters=100, ϵ=1e-1):
        ...
        for i in range(max_iters):
            old_γs, old_ms, old_βs, old_Ws, old_νs = self.γs, self.ms, self.βs, self.Ws, self.νs
            ...
            if np.linalg.norm(old_γs - np.array(self.γs)) + np.linalg.norm(old_ms - np.array(self.ms)) + np.linalg.norm(old_βs - np.array(self.βs)) \
    ...
    def _update_params(self):
        ...
        # optimising the weight distribution params γ
        for k in range(self.num_gaussians):
            self.γs[k][0] = 1 + Ns[k]
            self.γs[k][1] = self.α0 + np.sum(self.rs[:,k+1:])

        # optimising the component params (m, β, ν, W)
